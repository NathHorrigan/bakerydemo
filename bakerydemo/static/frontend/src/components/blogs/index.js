      ...
      <>
        <div className={styles.blogsFilters}>
          {this.renderTags(this.props.blogs).map(item => {
            return (
              <a href="#" className={styles.blogsFilter} onClick={event => this.handleFilterClick(event)}
                 key={item}>{item}</a>
            )
          })}
          {this.state.filtered ? (
          ...
          {this.state.activeBlogs.map(({node}) => {
            return (
              <Link to={`/blog/${node.slug}`} key={node.id}
                    className={this.state.filtered ? `${styles.blogs} ${styles.blogsFiltered}` : styles.blogs}>
                <Img className={styles.blogsImage} resolutions={node.image.localFile.childImageSharp.resolutions}
                     alt=""/>
